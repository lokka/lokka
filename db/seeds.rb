...
Site.create!(:title => "Test Site",
             ...
             :updated_at => updated_at) && puts("site successfly created") if Site.all.blank?
...
User.create!(:name => "test",
             ...
             :permission_level => 1,
             :created_at => created_at,
             :updated_at => updated_at) && puts("user successfly created") if User.all.blank?
...
Entry.create!(:user_id => 1,
              ...
              :type => "Post",
              :created_at => created_at,
              :updated_at => updated_at) && puts("entry successfly created") if Entry.all.blank?

Snippet.create!(:id => 1,
                :name => "Test Snippet",
                :body => "Text for test snippet.",
                :created_at => created_at,
                :updated_at => updated_at) && puts("snippett successfly created") if Snippet.all.blank?

Tag.create!(:id => 1,
            :name => "lokka") && puts("tag successfly created") if Tag.all.blank?

Tagging.create!(:id => 1,
                :taggable_id => 1,
                :taggable_type => "Entry",
                :tag_context => "tags",
                :tag_id => 1) && puts("tagging successfly created") if Tagging.all.blank?
